...
def generate_plots(trans_array, trans_array_time, td_f, td_dR, title, filename):
    ...
    log.info("np_shape  ****** = %s", np.shape(trans_array))
    min_depth = 0
    min_time = 0
    ...
    #trans_array_time = np.array(trans_array_time)

    #log.info("generate_plots: ******   trans_array_time = %s", trans_array_time)
    ...
    log.info("xticks *** = %s ", xticks)
    log.info("trans_array_time shape = %s", np.shape(trans_array_time))
